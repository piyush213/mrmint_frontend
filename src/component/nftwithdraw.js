...
const Nftwithdraw = () => {
    ...
    const [withdrawFeePer, setWithdrawFeePer] = useState(0);
    ...
    const getWalletBalanceAPI = async () => {
        ...
        if (res.success) {
            setWithdrawFeePer(res.data?.referral_withdrawal_fee_percent);
            setWithdrawFee(res.data?.mnt_withdrawal_fee_percent);
            setWalletBalance(res.data?.nft_usd_balance);
            setMiningWalletBalance(res.data?.main_balance);
                                ...
                                <div className="row justify-content-center">
                                    ...
                                    <div className="col-xxl-6 col-xl-6 col-lg-6 col-md-6 col-sm-6 col-12 pt-5">
                                        <div className='buyform withdraw'>
                                                ...
                                                <div className=''>
                                                    ...
                                                    <div className='form-group mb-3'>
                                                        ...
                                                        <select class="form-control" onChange={inputHandlerForType} name='withdraw_type'>
                                                            ...
                                                            <option className='withdrawType' value={2}>Main Wallet</option>
                                                        </select>
                                                    </div>
                                            ...
                                            <small class="text-white mb-3"> Minimum Withdraw = {withdrawType == 1 ? `$${referralMinimumAmount}` : `${miningRewardMinimumAmount} MNT`}</small> <br />

                                            {withdrawType == 2 ?
                                                <>
                                                    <small class="text-white mb-3"> Withdraw Fee: 2% </small><br />
                                                    <small class="text-white mb-3"> Tax Fee: 1% </small><br />
                                                    <small class="text-white mb-3"> You will get: {usdAmount ? parseFloat(parseFloat(usdAmount) - parseFloat(usdAmount) * parseFloat(withdrawFee) / 100).toFixed(2) : 0} MNT </small> </>
                                                :
                                                <>
                                                    <small class="text-white mb-3"> Withdraw Fee: {withdrawFeePer}% </small><br />
                                                    <small class="text-white mb-3"> You will get: ${usdAmount ? parseFloat(parseFloat(usdAmount) - parseFloat(usdAmount) * parseFloat(withdrawFeePer) / 100).toFixed(2) : 0} </small> </>
                                            }

                                            <div className="sc-CtfFt bxUreM mt-3 w-100" id="token-buy-button" onClick={withdrawNow} >Withdraw Now</div>
                                            <span style={{ fontSize: '14px', fontWeight:'bold' }}>Note: Withdrawal requests will be approved at 12 PM and 7 PM Everyday.</span>
                                        </div>
                                    </div>
