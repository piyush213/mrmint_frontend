...
const Staking = () => {
    ...
    const maxAmountSet = async () => {
        let maxAmount = 0;
        if (form.wallet === 'mnt') maxAmount = parseFloat(userDetails.mnt_balance).toFixed(2);
        else maxAmount = parseFloat(userDetails.vesting_balance).toFixed(2);
        setForm((old) => {
    ...
    const SubmitForm = async (e) => {
        ...
        if (!isNaN(form.amount) === false) {
            toast.error("Amount is not valid please enter valid amount.");
            return;            
        }        
        ...
        let inputAmount = parseFloat(form.amount).toFixed(2);

        if (parseFloat(maxAmount) < parseFloat(inputAmount)) {
    ...
    const columns = [
        ...
        },
        {
            ...
            cell: (item) => {
                return (
                    `${item.period - item.remaining} Months`
                );
            }
        },
        {
                                ...
                                <div className="row justify-content-center">
                                    <div className="col-xxl-6 col-xl-6 col-lg-6 col-md-6 col-sm-6 col-12 pt-5">
                                        <div className='buyform staking'>
                                                ...
                                                <div className=''>
                                                    <div className='form-group'>
                                                        <select className='form-control' onChange={inputHandler} name="wallet">
                                                            ...
                                                            <option value="mnt">MNT Wallet ({parseFloat(userDetails.mnt_balance).toFixed(2)} MNT)</option>
                                                            <option value="vesting">Vesting Wallet ({parseFloat(userDetails.vesting_balance).toFixed(2)} MNT)</option>
                                                        </select>
                                            ...
                                            {isProcess ?
                                                <div className="sc-CtfFt bxUreM" id="token-buy-button" >Processing...</div>
                                                :
